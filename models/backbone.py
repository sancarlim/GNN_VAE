...
def trim_network_at_index(network: nn.Module, index: int = -1, freeze: int = 5) -> nn.Module:
    """
    ...
    network = nn.Sequential(*list(network.children())[:index])
    ct=0
...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, version: str, freeze: int):
        """
        ...
        self.backbone = trim_network_at_index(RESNET_VERSION_TO_MODEL[version](pretrained=True), -1, freeze)
...
class MobileNetBackbone(nn.Module):
    ...
    def __init__(self, version: str, freeze: int):
        ...
        self.backbone = trim_network_at_index(mobilenet_v2(pretrained = True), -1, freeze)
