...
def process_tracks(tracks,  start_frame, end_frame, current_frame,sample_token):
    '''
...
def process_scene(scene, samples, instances):
    '''
    ...
    assert tracks[-1]['frame_id'][0] - tracks[0]['frame_id'][0] == len(frame_id_list)-1, f"{ tracks[-1]['frame_id'][0] - tracks[0]['frame_id'][0]} != {len(frame_id_list)} in scene {scene_id},{scene_token}"
    ...
    for i, [frame, track] in enumerate(tracks_per_frame):
        ...
        if sample_token in samples:
            ...
            #HD MAPs
            #maps = [transform(input_representation.make_input_representation(instance, sample_token)) for instance in tracks[current_ind]["node_id"]]   #Tensor [N_agents,3,112,112] float32 [0,1]       
...
#scenes_df=[]
#nuscenes.field2token('scene', 'name','scene-0')
...
for data_class in ['train']:
    ...
    for ann in ns_scene_names[data_class]:
        ...
        instances.add(instance_token)   #Instances in prediction_challenge.json 789
        samples.add(sample_token)  #3076
    all_data = []
    all_adjacency = []
    all_mean_xy = []
    all_tokens = []
    ...
    for scene_token in scenes_token_set:
        all_feature_sc, all_adjacency_sc, all_mean_sc, tokens_sc = process_scene(nuscenes.get('scene', nuscenes.field2token('scene', 'name','scene-0278')[0]), samples, instances)   # 780 scene_token = '656bb27689dc4e9b8e4559e3f6a7e534'
